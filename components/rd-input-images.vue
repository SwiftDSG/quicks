...
<script lang="ts" setup>
  ...
  function dragEnterHandler(e: DragEvent): DragEvent {
    e.preventDefault();
    inputDragging.value = true;
    return e;
  }
  function dragLeaveHandler(e: DragEvent): DragEvent {
    e.preventDefault();
    inputDragging.value = false;
    return e;
  }
  function dropHandler(e: DragEvent): DragEvent {
    e.preventDefault();
    inputDragging.value = false;
  ...
  function removeImage(e?: MouseEvent): MouseEvent {
  ...
  onMounted(() => {
    setTimeout(async () => {
      if (props.input.file?.length) {
        inputFiles.value = await Promise.all(
          props.input.file.map(
            (a): Promise<ImageFile> =>
              new Promise(async (resolve) => {
                ...
                if (typeof a === "string") {
                  const match: string[] = a.match(
                    /data:([a-zA-Z0-9]+\/[a-zA-Z0-9-.+]+).*,.*/
                  );
                  if (match?.length) {
                    const response: Response = await fetch(a);
                    const buffer: ArrayBuffer = await response.arrayBuffer();
                    const file: File = new File(
                      [buffer],
                      `${genRandomStr()}.${match[1].split("/")[1]}`,
                      { type: match[1] }
                    );
                    payload.file = file;
                  }
