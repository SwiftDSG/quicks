<template>
  <div class="rd-input-component">
    ...
    <input
      ...
      :accept="input.accept || 'image/svg+xml'"
    />
    <label
      ref="rdInputArea"
      :for="inputId"
...
<script lang="ts" setup>
  ...
  function inputHandler(state: "show" | "hide", e?: MouseEvent): MouseEvent {
    if (state === "show") {
  ...
  watch(
    ...
    () => {
      ...
      props.input.width = inputFile.value.width;
      props.input.height = inputFile.value.height;
    }
  );

  onMounted(() => {
    inputId.value = genRandomStr();
    setTimeout(() => {
